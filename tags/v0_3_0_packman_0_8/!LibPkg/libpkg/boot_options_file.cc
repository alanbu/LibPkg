 ...
 *
 * If it's not there is should be inserted after the section
 * |Start RISCOS !Boot 0.01 ResApps
 *
 * Entries should be added with the path relative to boot if possible
 *
 * Format of an entry is
 * IfThere HostFS:$.Dev.!GCC Then AddApp HostFS:$.Dev.!GCC
 *
 * or if it was on the boot drive
...
void boot_options_file::commit()
{
	...
	if (_modified)
	{
		...
		file.open(tmp_pathname.c_str(), std::ios_base::trunc);
		...
		file.close();
		if (!file) throw commit_error();

		write_filetype(tmp_pathname, 0xfea); // Set as desktop file
...
boot_options_file::commit_error::commit_error():
	runtime_error("failed to commit boot options file")
{}
