...
string canonicalise(const string& pathname)
{
	...
	libpkg::os::OS_FSControl37(pathname.c_str(),0,0,0,size,&size);
	//std::cout << std::hex << " size=" << size << std::endl;
	size=1-size;

	// Canonicalise pathname.
	char buffer[size];
	libpkg::os::OS_FSControl37(pathname.c_str(),buffer,0,0,size,&size);
...
void force_delete(const string& pathname)
{
	...
	try
	{
		...
		libpkg::os::OS_File4(pathname.c_str(),0x33);

		// Delete file.
		//std::cout << "deleting, ";
		libpkg::os::OS_File6(pathname.c_str(),0,0,0,0,0);
	}
	catch (...) {}
	...
	bool overwrite)
{
	...
	if (overwrite)
	{
		...
		try
		{
			...
			libpkg::os::OS_File4(dst_pathname.c_str(),0x33);

			// Delete file.
			//std::cout << "force_move: Delete dest" << std::endl;
			libpkg::os::OS_File6(dst_pathname.c_str(),0,0,0,0,0);
		}
		catch (...) {}
...
void copy_object(const string& src_pathname,const string& dst_pathname)
{
	...
	libpkg::os::OS_FSControl26(src_pathname.c_str(),dst_pathname.c_str(),
		0x0201,0,0,0);
}
...
void create_directory(const string& pathname)
{
	...
	libpkg::os::OS_File8(pathname.c_str(),0);
}
...
unsigned int object_type(const string& pathname)
{
	...
	libpkg::os::OS_File17(pathname.c_str(),&objtype,0,0,0,0);
	return objtype;
}
...
unsigned int object_length(const string& pathname)
{
	// Read object length.
	unsigned int length;
	libpkg::os::OS_File17(pathname.c_str(),0,0,0,&length,0);
	return length;
}
